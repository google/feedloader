...
/**
 * @fileoverview Query string exports for use in bq-stage-changes
 */

// This query selects the latest unique date from the streaming_items table.
const LATEST_DATE_SUBQUERY = `
  SELECT
    DISTINCT(import_datetime)
  FROM
    ${process.env.BQ_DATASET}.streaming_items
  ORDER BY
...
exports.COUNT_DELETES_QUERY = `
  SELECT COUNT(*) FROM ${process.env.BQ_DATASET}.items_to_delete
`;
...
exports.CALCULATE_ITEMS_FOR_UPDATE_QUERY = `
  SELECT
    Items.item_id
  FROM
    ${process.env.BQ_DATASET}.items AS Items
  INNER JOIN
    ${process.env.BQ_DATASET}.streaming_items AS Streaming
    ON
...
// batch of imported items, and selects their IDs.
exports.CALCULATE_ITEMS_FOR_INSERTION_QUERY = `
  SELECT
      CurrentRun.item_id
    FROM
      (
...
exports.COUNT_UPSERTS_QUERY = `
  SELECT COUNT(*) FROM ${process.env.BQ_DATASET}.items_to_upsert
`;
...
exports.GET_EXPIRING_ITEMS_QUERY = `
  SELECT
    E.item_id
  FROM
    ${process.env.BQ_DATASET}.items_expiration_tracking AS E
    INNER JOIN
    ${process.env.BQ_DATASET}.items AS I
    USING(item_id)
  WHERE
    DATE_DIFF(CURRENT_DATE('${
    process.env.TIMEZONE_UTC_OFFSET}'), E.last_touched_date, DAY) >= ${
    process.env.EXPIRATION_THRESHOLD}
    AND E.item_id NOT IN (SELECT item_id FROM ${
    process.env.BQ_DATASET}.items_to_upsert);
`;

// This query counts how many items were staged for expiration prevention and
// this number will be passed onto GAE via PubSub.
exports.COUNT_EXPIRING_QUERY = `
  SELECT COUNT(*) FROM ${process.env.BQ_DATASET}.items_to_prevent_expiring
`;
...
exports.DELETE_LATEST_STREAMING_ITEMS = `
  DELETE FROM ${process.env.BQ_DATASET}.streaming_items
  WHERE
    import_datetime = (${LATEST_DATE_SUBQUERY})
`;
