...
class ResultRecorder(object):
  ...
  def insert_result(self, operation: str, result: process_result.ProcessResult,
                    timestamp: str, batch_id: int) -> None:
    ...
    """
    self._insert_count_result(operation, result, batch_id, timestamp)
    self._insert_item_result(operation, result, batch_id, timestamp)

  def _insert_count_result(self, operation,
    ...
    if not response:
      logging.info(
          'Operation %s timestamp %s batch #%d: The result of the Content API for Shopping call was successfully recorded to BigQuery. Success inserting into table %s.',
          operation, timestamp, batch_id, table.table_id)
    else:
      logging.error(
          'Operation %s timestamp %s batch #%d: The result of the Content API for Shopping call was not recorded to BigQuery. Failure inserting into table %s. Results: %s.',
          operation, timestamp, batch_id, table.table_id,
          result.get_counts_str())

  def _insert_item_result(self, operation, result: process_result.ProcessResult,
    ...
    try:
      ...
      if not response:
        logging.info(
            'Operation %s timestamp %s batch #%d: The per item results of the Content API for Shopping call were successfully recorded to BigQuery. Success inserting into table %s.',
            operation, timestamp, batch_number, table.table_id)
      else:
        logging.error(
            'Operation %s timestamp %s batch #%d: The per item results of the Content API for Shopping call were not recorded to BigQuery. Failure inserting into table %s. Results: %s.',
            operation, timestamp, batch_number, table.table_id,
            result.get_ids_str())
    except cloud_exceptions.GoogleCloudError as google_cloud_error:
      logging.error(
          'Operation %s timestamp %s batch #%d: The per item results of the Content API for Shopping call were not recorded to BigQuery. Received a Google Cloud exception while trying to insert into table %s. Exception: %s. Results: %s.',
          operation, timestamp, batch_number, table.table_id,
          google_cloud_error, result.get_ids_str())
    except api_exceptions.GoogleAPIError as google_api_error:
      logging.error(
          'Operation %s timestamp %s batch #%d: The per item results of the Content API for Shopping call were not recorded to BigQuery. Received an API exception from while trying to insert into table %s. Exception: %s. Results: %s.',
          operation, timestamp, batch_number, table.table_id, google_api_error,
          result.get_ids_str())
